...
export default function VideoChatWrapper({ token }) {
  ...
  if (!authorized) return (
    <div className="p-6 bg-red-50 border border-red-200 rounded-lg">
      ...
      <div className="text-red-600"> You are not authorized to join this consltation </div>
    </div>
  )

  const roomName = `tbcare-appointment-${appointment.id}`
  const displayName = token?.user?.user_metadata?.full_name || 'Guest'

  return <VideoChat roomName={roomName} displayName={displayName} />
}
